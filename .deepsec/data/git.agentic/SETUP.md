# Agent setup for `git.agentic`

This is a deepsec scanning workspace. Project `git.agentic` was just registered
(target: `..`). Setup is incomplete — `data/git.agentic/INFO.md`
...
2. **Fill in `data/git.agentic/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id git.agentic
pnpm deepsec process --project-id git.agentic
```
