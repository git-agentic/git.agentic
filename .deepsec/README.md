...
Currently configured project: `git.agentic` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/git.agentic/SETUP.md` to fill in
   `data/git.agentic/INFO.md`.
...
`--project-id git.agentic` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/git.agentic/`.
